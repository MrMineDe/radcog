...
# Paths
input_images_dir = "datasets/setv6p/images"  # Folder containing original images
input_labels_dir = "datasets/setv6p/labels"  # Folder containing YOLO labels
...
transform = A.Compose([
    ...
    #A.CLAHE(clip_limit=2, tile_grid_size=(8,8), p=0.3),  # **Boosts contrast in low light**
     A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=0.5),  # Adjusts **brightness** MORE
     A.Rotate(limit=200, p=0.5, border_mode=cv2.BORDER_CONSTANT),  # Adds **rotation jitter** RESIZE BOX
     A.HorizontalFlip(p=0.5),  # **Flips images horizontally**
     A.RandomScale(scale_limit=1, p=0.3),  # Randomly scales the image MORE RELIABLE
